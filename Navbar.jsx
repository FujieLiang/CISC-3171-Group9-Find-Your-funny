      ...
      {open && (
        <div className="md:hidden border-t border-dim bg-paper/95 backdrop-blur-xl">
          <div className="px-4 py-4 flex flex-col gap-1">
            ...
            <NavLink to="/events" onClick={() => setOpen(false)} className="py-2.5 font-sans text-sm font-medium text-stark" data-testid="mobile-nav-events">Browse</NavLink>
            <NavLink to="/near-me" onClick={() => setOpen(false)} className="py-2.5 font-sans text-sm font-medium text-stark" data-testid="mobile-nav-nearme">Near Me</NavLink>
            {user && <NavLink to="/create-event" onClick={() => setOpen(false)} className="py-2.5 font-sans text-sm font-medium text-stark" data-testid="mobile-nav-create">Post Show</NavLink>}
            {user && <NavLink to="/profile" onClick={() => setOpen(false)} className="py-2.5 font-sans text-sm font-medium text-stark" data-testid="mobile-nav-profile">My Bills</NavLink>}
            <div className="hairline my-2" />
            {!user ? (
              <>
                <Link to="/login" onClick={() => setOpen(false)} className="py-2.5 font-sans text-sm font-medium text-stark" data-testid="mobile-nav-login">Log in</Link>
                <Link to="/register" onClick={() => setOpen(false)} className="btn-sm text-center mt-1" data-testid="mobile-nav-register">Get on the list</Link>
              </>
            ) : (
              <button onClick={() => { handleLogout(); setOpen(false); }} className="btn-sm text-center" data-testid="mobile-nav-logout">Log out</button>
            )}
          </div>
        </div>
      )}
